...
function seedFields(product: Product): ProductFieldValues {
  return {
    name: product.name,
    active: product.active,
    stock: String(product.stock),
    // Strip leading currency symbol (e.g. "€49.50" → "49.50")
...
export interface ProductEditContentProps {
  product: Product
...
export default function ProductEditContent({ product, onCancel, onSave }: ProductEditContentProps) {
  // Seeded once on mount — this component only mounts while in edit mode, so
  // re-entering edit (after returning to view) re-seeds from the current product.
  const [values, setValues] = useState<ProductFieldValues>(() => seedFields(product))
  ...
  const title: ReactNode = (
    <span className="flex flex-col gap-0.5">
      ...
      <span className="font-sans text-base font-medium text-black">Edit {product.name}</span>
    </span>
  )
